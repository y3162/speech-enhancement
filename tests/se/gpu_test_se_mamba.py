...
class SeMambaGpuTest(unittest.TestCase):
    def test_forward_and_mask_times_noisy_mag(self) -> None:
        device = torch.device("cuda")
        cfg = load_config(se_config("se_mamba"))
        model = SEMamba(cfg.model, cfg.data.stft.n_fft).to(device).eval()
