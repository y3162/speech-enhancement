...
def validate(generator, discriminator, loader, cfg, device) -> dict[str, float]:
    ...
    for clean_audio, noisy_audio, lengths in loader:
        ...
        losses = generator_loss(clean, clean_audio, gen, audio_g, gen_hat, metric_g, cfg.train.loss)
...
def main() -> None:
    ...
    if rank == 0:
        log(f"MP-SENet: {sum(p.numel() for p in generator.parameters()) / 1e6:.3f}M params, "
    ...
    for epoch in range(start_epoch, cfg.train.epochs):
        ...
        for clean_audio, noisy_audio in train_loader:
            ...
            metric_g = discriminator(clean.mag, gen_hat.mag)
            losses = generator_loss(clean, clean_audio, gen, audio_g, gen_hat, metric_g, w)
